...
| 段階 | 状態 | 入口 |
|------|------|------|
| 1 憲法 | 完了 | `specs/L1/`, `docs/`, `docs/templates/` |
| 2 OS | 完了 | `.cursor/skills/modes/`, `.cursor/rules/`, `/tdd` |
| 3 Template | 完了 | `apps/`, `infra/`, `sandbox/`, `input/` |
...
- `pnpm typecheck` — OK
- `terraform -chdir=infra validate` — OK（コミット前に `infra/main.tf` を有効 HCL へ修正済み）

## 既知ギャップ（続きの候補）

1. Prisma: schema あり、実行は `InMemoryTaskRepository`（`docs/examples-verify-demo.md`）
2. JWT: `apps/api/src/auth.ts` は単体テストあり、HTTP 未配線
