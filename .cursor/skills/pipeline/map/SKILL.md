---
name: map
description: specs と product の hub（PBI/Epic）を作り、必要なら cut へつなぐ。
...
1. **枝を決める。** 次のいずれか一つに限定する: `map`（PBI 新規／更新）/ `epic` / `reconcile`（issue 先行の後付け）。issue 切出しは `/cut`。完了条件: 枝名と対象パス（PBI / Epic）が一文で揃っている。
