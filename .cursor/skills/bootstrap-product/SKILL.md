...
3. **`product/` へ展開する。** 詳細マップは [reference.md](./reference.md) の「展開マップ」。既存の空 `product/apps` README は上書きしてよい。`product/sandbox/` は残す。モノレポ根（`package.json` / `pnpm-workspace.yaml` 等）は **`product/` 直下**に置く。`infra/` は example に含まれる場合のみ展開し、空スタブは作らない。完了条件: `product/` にアプリ実装と `product/package.json` が存在する（`product/infra` は必須ではない）。
