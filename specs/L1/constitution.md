...
4. **`specs/` はソースを参照しない**: `specs/` 配下の文書は、実装ソース（`product/` および `sandbox/`）へのパス・ファイル名リンクを置かない。規範の根拠を実装ファイルにしない。許可するのは他仕様・ADR・`specs/inbox`（Source）・`issues` / `pbl`（ハブ）。実装↔仕様のトレースは issue / PBI / ADR / inbox 側に置き、L2/L3 の規範本文は下を向かない。
